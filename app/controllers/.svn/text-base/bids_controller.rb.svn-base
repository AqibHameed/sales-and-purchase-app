class BidsController < ApplicationController
  ...
  def parcel_report
    ...
    past_tenders = Tender.find(:all, :conditions => ["id != ? and company_id = ? and date(open_date) < ?",@tender.id, @tender.company_id, @tender.open_date.to_date], :order => "open_date DESC")
    @past_tenders = past_tenders ? past_tenders.collect(&:id) : []
