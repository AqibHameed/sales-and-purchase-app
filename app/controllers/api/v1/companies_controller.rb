class Api::V1::CompaniesController < ApplicationController
  ...
  def blocked_customers
    if current_customer
      ...
      render json: { success: true, blocked_customers: blocked.map { |e| { id: e.try(:block_user).try(:id), company: e.block_user.try(:company), customer_name: e.block_user.try(:name), email: e.block_user.try(:email)}}, response_code: 200 }
    end
  end
