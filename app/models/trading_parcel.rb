class TradingParcel < ApplicationRecord
  ...
  def self.search_by_filters(params, current_customer)
    ...
    parcels = parcels.where(description: params[:description]) unless params[:description].blank?
    parcels = parcels.where(company_id: params[:company_id]) unless params[:company_id].blank?
